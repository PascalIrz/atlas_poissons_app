...
```{r}
# Menu déroulant sélection du bassin versant
selectInput(
  "bassin",
  "Choisissez un bassin versant",
  choices = sort(unique(bv_simp_geo$toponyme)),
  selected = 1
)

# Données bassin versant

bv_data$effectif <- as.integer(bv_data$effectif) #Pour enlever la virgule et les 0

liste_esp_bv <- reactive ({
  bv_data %>% 
    left_join(bv_simp_geo) %>% 
    filter(toponyme == input$bassin,
         effectif > 0) %>% 
    select(esp_nom_commun, effectif) %>% 
    distinct() %>% 
    rename("Espèce" = esp_nom_commun,
           "Abondance" = effectif)
})

# Affichage
renderTable ({ liste_esp_bv() })
