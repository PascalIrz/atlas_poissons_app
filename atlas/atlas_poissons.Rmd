...
code_especes <- pt_data %>% 
  pull(code_alternatif)

fiche_inpn <- reactive ({
  codes_especes %>%
    filter(esp_nom_commun == input$espece) 
  # %>% 
    # unique()  %>% # ne sert à rien on dirait
  # %>%
    ...
    # Technique 4
    # paste0("<a href='https://inpn.mnhn.fr/espece/cd_nom/",
    # codes_especes$code_alternatif,  "' target='_blank'>", "Fiche",
    # "</a>")
})
...
bv_select <- bv_data %>% 
  ...
  select(esp_nom_commun, effectif, toponyme)


# Menu sélection bv
selectInput("bassin",
            "Choisissez un bassin versant",
            choices = sort(unique(bv_select$toponyme)),
            selected = 1)
