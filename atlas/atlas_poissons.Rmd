...
# Lien vers la fiche INPN
renderUI ({
  HTML(
    infos_esp()$fiche_inpn
  )
})
...
#       selected <- bv_map_data()[bv_map_data()$code_exutoire == click()$id,]
#       # 
...
renderTable({
  bv_select()
      ...
      # # #subset your table with the id of the clicked polygon
      # selected <- bv_map_data()[bv_map_data()$code_exutoire == event()$id,]
      # #
      # # #if click id isn't null render the table
      # if(!is.null(event()$id)){
      #   output$mytable = DT::renderDataTable({
